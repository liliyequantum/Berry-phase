function strong_field_numerical_rqndc_function(alpha)
    maxNumCompThreads(6);
    ...
    p_x_n=linspace(-16,-0.0001,1600);
    p_x_p=-p_x_n(end:-1:1);
    p_y_n=linspace(-16,-0.0001,1600);
    p_y_p=-p_y_n(end:-1:1);
    dp=p_x_p(2)-p_x_p(1);
    p_max=p_x_p(end);
    p_min=p_x_p(1);
    ...
    J_upper=zeros(length(t),1);
    J_flat=zeros(length(t),1);
    start = tic;
    for k = 1:1:length(t)-1
        ...
        [J_p_intra,J_p_upper,J_p_flat] =J_p_intra_map(x,t(k+1),p_x,p_y);
        ...
        J_upper(k+1,1)=sum(J_p_upper)*dp.^2;
        J_flat(k+1,1)=sum(J_p_flat)*dp.^2;
        
        disp(strcat('progress:',num2str(k/(length(t)-1))))
    ...
    save([pwd,'/alpha_T3','_xi_',num2str(xi),'_alpha_',num2str(alpha_parameter), '_p_max_',num2str(p_max),'_p_min_',num2str(p_min),...
        ...
        'J_up_flat','J_up_lower','J_flat_lower','J_upper','J_flat','check_normalization',...
        'xi','alpha_parameter')
